...
/*   Updated: 2018/03/19 13:17:55 by fde-souz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_inter		ray_cylindre(t_env *e, int i, int j, t_cam ca, int nbr)
{
	...
	p.a = v.x * v.x + v.z * v.z;
	p.b = v.x * (ca.xr - e->obj[nbr].pos.x) + v.z * (ca.zr - e->obj[nbr].pos.z);
	p.c = (ca.xr - e->obj[nbr].pos.x) * (ca.xr - e->obj[nbr].pos.x) + (ca.zr - e->obj[nbr].pos.z) * (ca.zr - e->obj[nbr].pos.z) - e->obj[nbr].rayon * e->obj[nbr].rayon;
	delta = powf(p.b, 2) - p.a * p.c;
	if (delta >= 0)
	{
		if (delta > 0)
		{
