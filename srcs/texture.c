...
/*   Updated: 2018/04/12 17:50:01 by fde-souz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Uint32	get_texture_pixel(t_env *e, t_inter pt, t_obj obj)
{
	...
	v.y = v.y * e->test->h;
	v.x = v.x * e->test->w;
	printf("%d %d\n", e->test->w, e->test->pitch);
	v.color = v.pixels[(int)v.x * 3 + e->test->w * 3 * (int)v.y];
	v.color += v.pixels[(int)v.x * 3 + e->test->w * 3 * (int)v.y + 1] * 256;
	v.color += v.pixels[(int)v.x * 3 + e->test->w * 3 * (int)v.y + 2]
	* 256 * 256;
	return (v.color);
}
