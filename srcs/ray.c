...
/*   Updated: 2018/03/05 15:49:32 by vgauther         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	one_pixel(t_obj obj, t_sdl *sdl, int i, int j, t_cam ca)
{
	...
	f1 = ca.xr - obj.sphere[0].pos.x;
	f2 = ca.yr - obj.sphere[0].pos.y;
	f3 = ca.zr - obj.sphere[0].pos.z;
	vx = (i - SIZE_X / 2);
	vy = (j - SIZE_Y / 2);
	vz = (SIZE_X / 2) / tan(30 * RAD);
	p.a = (vx * vx) + (vy * vy) + (vz * vz);
	p.b = 2 * (vx * (f1) + vy * (f2) + vz * (f3));
	p.c = (((f1 * f1) + (f2 * f2) + (f3 * f3)) - obj.sphere[0].rayon_pow);
